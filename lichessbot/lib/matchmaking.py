...
daily_challenges_file_name = "lichessbot/daily_challenge_times.txt"
...
class Matchmaking:
    ...
    def get_weights(
        self,
        online_bots: list[UserProfileType],
    ...
    def choose_opponent(self) -> tuple[Optional[str], int, int, int, str, str]:
        ...
        override = (
            ...
            else self.matchmaking_cfg.overrides.lookup(override_choice)
        )
    ...
    def get_random_config_value(
        self, config: Configuration, parameter: str, choices: list[str]
    ) -> str:
    ...
    def challenge(
        ...
        """
        max_games_for_matchmaking = (
            max_games if self.matchmaking_cfg.allow_during_games else min(1, max_games)
        )
    ...
    def show_earliest_challenge_time(self) -> None:
        ...
        if self.matchmaking_cfg.allow_matchmaking:
            ...
            logger.info(
                f"Next challenge will be created after {earliest_challenge_time.strftime('%X')} "
