    ...
    def search(self, board, time_limit, ponder, draw_offered, root_moves):
        ...
        try:
            with torch.no_grad():
                ...
                bin_indices = outputs.argmax(dim=1).cpu()  # move to CPU immediately

            # Free GPU tensors right away
            del fen_tokens_batch, move_tokens_batch, outputs
        ...
        if len(board.move_stack) in (0, 1):
            ...
            opening_top_n = self.opening_top_n_w if len(board.move_stack) == 0 else self.opening_top_n_b
            ...
            logger.info(f"Athena opening sampling from top {len(candidate_moves)} moves: {chosen_move.uci()}")
