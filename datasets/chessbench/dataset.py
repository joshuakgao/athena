...
bag = BagReader("behavioral_cloning_data.bag")
data = bag[0]
TYPE = "behavioral_cloning"
print(data)

if TYPE == "action_value":
    # Split the bytes into parts
    parts = data.split(b"\x04")

    # The first part is the FEN string (skip the initial colon)
    fen = parts[0][1:].decode("utf-8")

    # The second part contains move and win probability
    move_win_part = parts[1]

    # The move appears to be before the '?' character
    move_part = move_win_part.split(b"?")[0]
    move = move_part.decode("utf-8")

    # The remaining part seems to contain the win probability
    # This appears to be some binary representation that needs interpretation
    # We'll extract the bytes after '?' as the win_prob
    win_prob_bytes = move_win_part.split(b"?")[1]

    # Decode the win probability bytes as a float
    win_prob = struct.unpack("<f", win_prob_bytes[:4])[0]

    print("FEN:", fen)
    print("Move:", move)
    print("Win probability bytes:", win_prob)
elif TYPE == "state_value":
    # Split the bytes into parts
    parts = data.split(b"?")

    # The first part contains the FEN (skip the initial '%')
    fen = parts[0][1:].decode("utf-8")

    # The second part contains the win probability bytes
    win_prob_bytes = parts[1]

    win_prob = struct.unpack("<f", win_prob_bytes[:4])[0]

    print("FEN:", fen)
    print("Win probability bytes:", win_prob)
elif TYPE == "behavioral_cloning":
    fen_end = -4  # The move is the last 4 characters
    fen = data[1:fen_end].decode("utf-8")

    # Extract move - last 4 characters
    move = data[-4:].decode("utf-8")
