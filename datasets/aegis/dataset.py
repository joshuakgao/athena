...
class AegisDataset(Dataset):
    ...
    def sample_dataset(self):
        if self.test:
            ...
            self.data = pd.read_parquet(
                self.test_path, columns=["fen", "best_move", "eval"]
            )
        else:
            # Load a sampled subset of the training set
            self.data = pd.DataFrame(columns=["fen", "best_move", "eval"])
            ...
            for shard_path in tqdm(
                ...
                df = pd.read_parquet(shard_path, columns=["fen", "best_move", "eval"])
                num_rows = df.shape[0]
    ...
    def __getitem__(self, idx):
        ...
        v = self.encode_eval([eval])
        return x[0], y[0], v[0], fen, eval

    def encode_position(self, fens):
        """
        Encode a batch of FEN strings into 18×8×8 format.
        The planes are ordered as follows:
        - 0-11: Piece presence (12 planes)
        ...
        - 17: Just a full plane of 1s, used to prevent fuzzy borders of cnn
        """
        B = len(fens)
        planes = np.zeros((B, 18, 8, 8), dtype=np.float32)  # Now only 21 planes needed
        ...
        for b_idx, fen in enumerate(fens):
            ...
            planes[b_idx, 12, :, :] = 1.0 if board.turn == chess.WHITE else 0.0
    ...
    def encode_eval(self, evals, scale=400):
        """
    ...
    def decode_eval(self, encoded_evals, scale=400):
        """
